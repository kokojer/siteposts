	...
	try {
		...
		if (req.cookies.username === req.params.username) {
			...
			return res.render(createPath("userPage"), {
				...
				profileuserImg: user.img || "https://i.ibb.co/zR9nG57/anonimous.jpg",
...
const getEditUser = async (req, res) => {
	...
	try {
		...
		return res.render(createPath("userEditPage"), {
			...
			profileusername: user.username,
			profilenickname: user.nickname,
			profileuserImg: req.cookies.img || user.img || "https://i.ibb.co/zR9nG57/anonimous.jpg",
			profileage: user.age,
			profilesex: user.sex,
...
const addImg = async (req, res) => {
	try {
		if (req.allowed === true) {
			...
			let up = await uploadImg(req.file.filename) || 'https://i.ibb.co/zR9nG57/anonimous.jpg'
			if (up === 'https://i.ibb.co/zR9nG57/anonimous.jpg') {
				console.log('412');
